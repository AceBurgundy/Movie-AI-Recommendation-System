...
def save_picture(location, image, image_quality=80, as_thumbnail=True) -> str:
    ...
    """
    random_value: str = secrets.token_hex(8)
    new_file_name: str = random_value + '.webp'
    file_path: str = os.path.join(current_app.root_path, location, new_file_name)

    image_instance: Image = Image.open(image)

    if as_thumbnail:
        output_size: Tuple[int] = (125, 125)
...
class CheckProfanity():
    ...
    Args:
    -----
        message (str, optional): The error message to display when vulgar words are found.
            Defaults to "not saved. Vulgar word/s found".
        enumerate_words (bool, optional): Whether to show all vulgar words found.
            Defaults to False.
    ...
    def __call__(self, form, field) -> Optional[Exception]:
        """
        Validate the input field for vulgar words.

        Raises:
        -------
            ValidationError: If vulgar words are found in the input field.
        """
        result: List[str] = [word for word in profanity 
                    if word in ("".join(word for word in str(field.data).lower() 
...
def timer(func):
    ...
    Args:
    -----
      func: The `func` parameter is a function that will be timed.
    
    Returns:
    --------
      The timer function returns a wrapper function.
    """
    def wrapper(*args, **kwargs):
        ...
        end_time: float = time()
        execution_time_ms: float = round((end_time - start_time) * 1000, 1)
        print(f"\n{func.__name__} took {execution_time_ms:.1f} milliseconds to execute.")
        return result
    return wrapper

def has_illegal_words(text, message=None, enumerate_words=False) -> Union[List[str], bool]:
    """
    Check if the given text contains illegal words.

    Args:
    -----
        text (str): The text to check for illegal words.
        message (str, optional): The error message to return if illegal words are found.
        enumerate_words (bool, optional): Whether to return all illegal words found.

    Returns:
    --------
        bool or list: False if no illegal words are found, or a list of illegal words if found.

    """
    result: List[str] = [word for word in profanity if word in text]
