...
class CsvAlchemy:
    ...
    def _get_csv(self, filename: str) -> str:
        ...
        """
        return path.abspath(f"Engine/recommender/dataset/{filename}")
    ...
    def _validate_data(self, search_dictionary) -> Union[Exception, None]:
        ...
        for key, value in search_dictionary.items():
            ...
            if str(value).strip() == '':
                message: str = f"Missing or wrong column data for the following query {json_dictionary}"
                print(message)
                raise self.ValidationException(message)
            
            if key not in self.columns:
                message: str = f"The following column_name {search_dictionary['column_name']} is not among this csv file's columns -> {', '.join(self.columns)}"
                print(message)
                raise self.ValidationException(message)
    ...
    def insert_row(self, data_dictionary: Dict[str, Union[str, Any]]) -> Union[SEARCH, Exception]:
        ...
        for column_name in data_dictionary:
            if column_name not in self.columns:
                message: str = f"Column '{column_name}' does not exist in the CSV file '{self.filename}'"
                print(message)
                raise self.InsertionException(message)
        ...
        try:
            ...
            return inserted_row
        except self.InsertionException as error:
            message: str = f"Failed to insert new data into CSV file: { str(error) }"
            print(message)
            raise self.InsertionException(message)

    def _retrieve(self, search_dictionary: dict) -> Union[Series, Exception]:
        """
        ...
        Returns:
        --------
            Series: Series with retrieved data.
        ...
        conditions = None
        for column_name, column_data in search_dictionary.items():
            if conditions is None:
                conditions = self.csv_data[column_name] == column_data
            else:
                conditions &= self.csv_data[column_name] == column_data

        row = self.csv_data[conditions]

        if row.empty:
            search_query = json.dumps(search_dictionary, indent=4)
            message = f"Row search by the following values {search_query} is empty"
            print(message)
            raise self.RowNotFoundException(message)
